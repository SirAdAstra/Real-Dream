...
public class SpriteMotion : MonoBehaviour
{
    ...
    private void SpriteMovment()
    {
        ...
        if (GameButtonsScript._publMusicStatus == 0)
        {
            note.transform.position += new Vector3(currentSpeed, 0, 0);
        }
        else if (GameButtonsScript._publMusicStatus == 1)
        {
    ...
    private void SpawnNewNote(Collider2D other, bool firstSpawn)
    {
        ...
        other.gameObject.GetComponent<GoalScript>().SpawnNewGoal(rand, firstSpawn);
        switch (rand)
        {
            case 0:
            {
    ...
    {
        int targetVert = (int)other.gameObject.GetComponent<GoalScript>().targetVert;
        int tarhetHor = (int)other.gameObject.GetComponent<GoalScript>().targetHor;
        int targetColor = (int)other.gameObject.GetComponent<GoalScript>().targetColor;
