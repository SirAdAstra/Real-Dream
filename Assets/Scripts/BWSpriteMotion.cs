...
public class BWSpriteMotion : MonoBehaviour
{
    ...
    private void SpriteMovment()
    {
        ...
        if (BWGameButtonsScript._publMusicStatus == 0)
        {
            note.transform.position += new Vector3(currentSpeed, 0, 0);
        }
        else if (BWGameButtonsScript._publMusicStatus == 1)
        {
    ...
    private void SpawnNewNote(Collider2D other, bool firstSpawn)
    {
        ...
        other.gameObject.GetComponent<BWGoalScript>().SpawnNewGoal(rand, firstSpawn);
        switch (rand)
        {
            case 0:
            {
    ...
    {
        int targetVert = (int)other.gameObject.GetComponent<BWGoalScript>().targetVert;
        int tarhetHor = (int)other.gameObject.GetComponent<BWGoalScript>().targetHor;
        int targetColor = (int)other.gameObject.GetComponent<BWGoalScript>().targetColor;
