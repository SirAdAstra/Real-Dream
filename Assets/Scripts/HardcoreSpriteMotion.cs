...
public class HardcoreSpriteMotion : MonoBehaviour
{
    ...
    [SerializeField] private Sprite[] greyNotes;
    ...
    private void Start()
    {
        ...
        SpawnNewNote(target.GetComponent<Collider2D>(), true);
    }
    ...
    private void SpriteMovment()
    {
        ...
        if (HardcoreGameButtonsScript._publMusicStatus == 0)
        {
            note.transform.position += new Vector3(currentSpeed, 0, 0);
        }
        else if (HardcoreGameButtonsScript._publMusicStatus == 1)
        {
    ...
    public void LightNotes()
    {
        StartCoroutine(LightNotesCoroutine());
    }

    private void SpawnNewNote(Collider2D other, bool firstSpawn)
    {
        int rand = Random.Range(0, 3);
        other.gameObject.GetComponent<HardcoreGoalScript>().SpawnNewGoal(rand, firstSpawn);
        switch (rand)
        {
            case 0:
            {
                noteColor = Color.Blue;
                note.GetComponent<SpriteRenderer>().sprite = greyNotes[0];
                break;
            }
            case 1:
            {
                noteColor = Color.Green;
                note.GetComponent<SpriteRenderer>().sprite = greyNotes[1];
                break;
            }
            ...
            {
                noteColor = Color.Red;
                note.GetComponent<SpriteRenderer>().sprite = greyNotes[2];
                break;
            }
        }
    ...
    {
        int targetVert = (int)other.gameObject.GetComponent<HardcoreGoalScript>().targetVert;
        int tarhetHor = (int)other.gameObject.GetComponent<HardcoreGoalScript>().targetHor;
        int targetColor = (int)other.gameObject.GetComponent<HardcoreGoalScript>().targetColor;

        if (targetVert == (int)noteVert && tarhetHor == (int)noteHor && targetColor == (int)noteColor)
        {
            completed += 1;
            SpawnNewNote(other, false);
    ...
    private IEnumerator LightNotesCoroutine()
    {
        switch ((int)noteColor)
        {
            case 0:
            {
